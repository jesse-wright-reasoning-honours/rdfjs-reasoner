 ...
 export class UnionIterator<T> extends AsyncIterator<T> {
   ...
   */
   constructor(sources: AsyncIteratorOrArray<AsyncIterator<T>> |
     ...
     // @ts-ignore
     this._sources = (Array.isArray(sources) ? fromArray(sources) : wrap(sources, { letIteratorThrough: true })).map<AsyncIterator<T>>((it: any) => isPromise(it) ? wrap(it as any, { letIteratorThrough: true }) : (it as AsyncIterator<T>));
...
// }xport function wrap<T>(source: EventEmitter | Promise<EventEmitter>, options?: TransformIteratorOptions<T>) {
//   //    return new TransformIterator<T>(source as AsyncIterator<T> | Promise<AsyncIterator<T>>, options);
//   //  }
