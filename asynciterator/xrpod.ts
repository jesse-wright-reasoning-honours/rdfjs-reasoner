...
export class XProd<A, B, C> extends AsyncIterator<C> {
  private i: number;
  ...
  constructor(private arr: A[], private iter: AsyncIterator<B>, private f: (a: A, b: B) => C | null) {
    super()
    this.i = arr.length - 1;
  ...
  read(): C | null {
    ...
    } while ((item = this.f(this.arr[++this.i], this.item!)) === null)
    return item;
  }
}
